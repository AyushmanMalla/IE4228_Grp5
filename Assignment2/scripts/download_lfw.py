...
Usage:
    python scripts/download_lfw.py [--output-dir data] [--source kaggle|sklearn|direct] [--min-faces 10]
"""
...
def _download_kaggle(output_dir: Path) -> Path:
    ...
    # Kaggle may nest files — find the folder containing person directories
    ...
    # Clean up temp download folder (if still exists)
...
def _find_lfw_root(search_dir: Path) -> Path:
    ...
    for root, dirs, files in os.walk(search_dir):
        root_path = Path(root)
        # A valid LFW root has many subdirectories, each containing images
        if len(dirs) > 100:  # LFW has 5749 identities
            return root_path
        # Also check for a folder literally named "lfw" or "lfw-deepfunneled"
        for d in dirs:
...
def _download_sklearn(output_dir: Path, min_faces: int = 10) -> Path:
    ...
    This is the most reliable method — sklearn maintains working mirrors.
    Downloads to sklearn's cache then copies into our folder structure.
    """
    ...
    for i, (image_data, label) in enumerate(zip(dataset.images, dataset.target)):
        ...
        # sklearn returns float64 images [0,1] — convert to uint8
...
# ---------------------------------------------------------------------------
# Source 3: Direct URL (original UMass — may be down)
# ---------------------------------------------------------------------------

DIRECT_URLS = [
    "https://ndownloader.figshare.com/articles/5976726/versions/1",  # figshare mirror
...
def _download_direct(output_dir: Path) -> Path:
    ...
    for url in DIRECT_URLS:
        ...
        try:
            ...
            if str(archive).endswith(".tgz") or str(archive).endswith(".tar.gz"):
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Directory to save the dataset (default: Assignment2/data)",
    )
    parser.add_argument(
        "--source",
